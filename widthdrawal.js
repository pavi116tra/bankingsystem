...
submit_inp.addEventListener("click", async function (e) {
  ...
  const withdrawStr = input1.value.trim();
  const withdrawAmount = parseInt(withdrawStr);

  if (isNaN(withdrawAmount) || withdrawAmount <= 0) {
    alert("Please enter a valid positive withdraw amount.");
    return;
  }

  // ✅ FIXED: Changed from loginacc_num to account_num
  ...
  try {
    ...
    const currentBalance = parseFloat(userData.amount);
